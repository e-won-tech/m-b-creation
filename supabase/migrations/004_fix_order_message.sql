-- แก้จริง: สร้างข้อความสรุปใน text variable ก่อน แล้วค่อยใส่ jsonb_build_object
-- เลี่ยงปัญหา "invalid input syntax for type json" ตอนประกอบข้อความไทยกับ case ฝังใน
...
begin
  ...
  -- สร้างข้อความเก็บใน text variable (ไม่ฝัง case ใน jsonb_build_object)
  v_message :=
...
exception
  when others then
    ...
    return jsonb_build_object('ok', false, 'error', 'DEBUG: ' || SQLERRM, 'sqlstate', SQLSTATE);
end;
$$;
