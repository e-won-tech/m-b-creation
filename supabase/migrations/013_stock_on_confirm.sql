-- ตัดสต็อกเมื่อยืนยันการชำระเงิน (confirmed ขึ้นไป) แทนการตัดตอนสร้างออเดอร์
-- และคืนสต็อกกลับเมื่อยกเลิก (cancelled)

-- 1) ธงกันตัด/คืนซ้ำ
alter table public.orders
  add column if not exists stock_committed boolean not null default false;

-- 2) create_order_from_cart เวอร์ชันใหม่ = ไม่ตัดสต็อกตอนสร้าง (ยังเช็คว่าพอ)
...
begin
  ...
  loop
    ...
    -- หมายเหตุ: ไม่ตัดสต็อกที่นี่แล้ว จะตัดตอนแอดมินยืนยันการชำระเงิน (ดู trigger ด้านล่าง)
...
-- 3) trigger: ตัดสต็อกเมื่อยืนยันชำระเงิน (confirmed/packing/shipped/completed), คืนเมื่อยกเลิก
...
declare
  r record;
begin
  -- เข้าสู่สถานะที่ถือว่าชำระเงินแล้ว → ตัดสต็อก (ครั้งเดียว)
  if new.status in ('confirmed', 'packing', 'shipped', 'completed')
     and not coalesce(old.stock_committed, false) then
    for r in select product_id, qty from public.order_items where order_id = new.id loop
  ...
  -- ยกเลิกออเดอร์ที่เคยตัดสต็อกไปแล้ว → คืนสต็อก
