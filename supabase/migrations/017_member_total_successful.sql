...
begin
  ...
  -- ค่าจัดส่ง Flash ขั้นต่ำ 60 บาท
  if v_method not like 'จัดส่งขนส่งเอกชน%' and v_method <> 'รับสินค้าด้วยตัวเอง' then
    v_shipping := greatest(v_shipping, 60);
  end if;

  v_grand := v_total + v_shipping;
  update public.orders set total = v_grand, shipping_fee = v_shipping where id = v_order_id;

  -- สร้าง/อัปเดตข้อมูลสมาชิก (ไม่บวกยอดสะสมที่นี่ — trigger จะคิดจากออเดอร์สำเร็จ)
...
-- 2) คำนวณยอดสะสมของสมาชิกใหม่จากออเดอร์สำเร็จ
create or replace function public.recalc_member_total(p_shop uuid, p_uid text)
returns void
language plpgsql
security definer
set search_path = public
...
create or replace function public.trg_member_total()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if tg_op = 'DELETE' then
    perform public.recalc_member_total(old.shop_id, old.line_user_id);
    return old;
  else
    perform public.recalc_member_total(new.shop_id, new.line_user_id);
    return new;
  end if;
end;
$$;

drop trigger if exists trg_orders_member_total on public.orders;
create trigger trg_orders_member_total
  after insert or update of status or delete on public.orders
  for each row execute function public.trg_member_total();

-- 3) backfill: คำนวณยอดสะสมของสมาชิกทุกคนใหม่ให้ตรงกับออเดอร์สำเร็จปัจจุบัน
update public.members m
   set total_spent = coalesce((
         select sum(o.total)
           from public.orders o
