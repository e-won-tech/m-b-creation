-- ชั่วคราว: ให้ create_order_from_cart คืนข้อความ error จริง (SQLERRM) แทนข้อความกลางๆ
-- ใช้เพื่อหาสาเหตุที่ "สร้างออเดอร์ไม่สำเร็จ" แล้วค่อยแก้ตามจริง
...
begin
  ...
  return jsonb_build_object(
    'ok', true, 'order_id', v_order_id, 'order_no', v_order_no, 'total', v_total,
    'message',
...
exception
  when others then
    ...
    -- คืน error จริง + context เพื่อ debug
    return jsonb_build_object('ok', false, 'error', 'DEBUG: ' || SQLERRM, 'sqlstate', SQLSTATE, 'detail', v_detail, 'context', v_ctx);
end;
$$;
